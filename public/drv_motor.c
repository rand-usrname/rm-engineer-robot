...
static void motor_angle_adjsut(rt_uint32_t angle,DjiMotor_t* motor)
{
	...
	if(motor->speed >= 0)
	{
		if((angle < motor->old_angle - 2000)&&(motor->old_angle > 4096))				/*加1000是为了去除编码器波动*/
		{
	...
	{
		if((angle > motor->old_angle + 2000)&&(angle > 4096))				/*加1000是为了去除编码器波动*/
		{
			motor->all_angle -= 8192; 
		}
	}
	/*计算0-8191范围内的角度*/
	angletemp = (motor->all_angle + angle) * motor->ratio / 8192;
