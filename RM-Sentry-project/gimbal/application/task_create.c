...
static void task_101ms_entry(void *parameter)
{
	while(1)
	{
...
void task_create(void)
{
	...
	thread = rt_thread_create("5ms_sem", task_5ms_entry, RT_NULL, 1024, 1, 1);
	if (thread != RT_NULL)
	{
			rt_thread_startup(thread);
	}
	...
	thread = rt_thread_create("101ms_sem", task_101ms_entry, RT_NULL, 1024, 1, 1);
	if (thread != RT_NULL)
	{
			rt_thread_startup(thread);
	}
